/*------------------------------------------------------------------------------
	"main.c" : fase 2 / progP, progG y progM

	Versión final de GARLIC 2.0
	(multiplexación, retardar procesos, matar procesos, escribir mensajes en 
	color, escribir caracteres, matrices de caracteres y carga de programas con 2 segmentos, listado de programas, gesti?n de
	franjas de memoria)
...
/*const char *progs[9] = {"BORR","CRON","DESC","HOLA","LABE","PONG","PRNT", "ORDI", "MDET"};
...
void seleccionarPrograma()
{
	...
	ind_prog = escogerOpcion((char **) progsAux, num_progs);
	_gg_escribir("%1*** Seleccionar argumento :\n", 0, 0, _gi_za);
	argumento = escogerOpcion((char **) argumentosDisponibles, 4);
	
	start = _gm_cargarPrograma(_gi_za, (char *) progsAux[ind_prog]);
	if (start)
	{	
		_gp_crearProc(start, _gi_za, (char *) progsAux[ind_prog], argumento);
		_gg_escribir("%2* %d:%s.elf", _gi_za, (unsigned int) progsAux[ind_prog], 0);
		_gg_escribir(" (%d)\n", argumento, 0, 0);
...
//------------------------------------------------------------------------------
	...
	{
		memcpy(progsAux, progs, sizeof(progs));
